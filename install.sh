...
if [ "$REMOTE_MODE" = true ]; then
    EXISTING_REMOTE_ENV_FILE="$CLAUDE_DIR/memento-remote.env"
    if [ -f "$EXISTING_REMOTE_ENV_FILE" ]; then
        # Generated by this installer as simple KEY="value" assignments.
        # Reuse it so existing remote installs can be re-run to add new MCP clients.
        set -a
        # shellcheck disable=SC1090
        . "$EXISTING_REMOTE_ENV_FILE"
        set +a
        REMOTE_URL="${REMOTE_URL:-${MEMENTO_VAULT_URL:-}}"
    fi
...
if [ ! -f "$CONFIG_DIR/memento.yml" ]; then
    ...
    if [ "$EXPERIMENTAL" = true ]; then
        EXPECTED_KEYS="$EXPECTED_KEYS session_briefing briefing_max_notes briefing_min_score prompt_recall recall_min_score recall_max_notes tool_context tool_context_min_score tool_context_max_notes tool_context_max_injections tool_context_cooldown tool_context_cache_ttl_hours tool_context_diagnostics tool_context_diagnostics_include_candidates tool_context_diagnostics_max_candidates multi_hop_enabled multi_hop_max"
    fi
...
if command -v codex &>/dev/null; then
    ...
    CODEX_SKILLS_SKIPPED=0

    INSTALL_CODEX_SKILLS="memento memento-defrag start-fresh continue-work concierge"
    if [ "$EXPERIMENTAL" = true ]; then
        INSTALL_CODEX_SKILLS="$INSTALL_CODEX_SKILLS inception"
    fi

    for skill in $INSTALL_CODEX_SKILLS; do
        mkdir -p "$CODEX_HOME_DIR/skills/$skill"
        if [ -f "$SCRIPT_DIR/skills/generic/$skill/SKILL.md" ]; then
            if safe_copy "$SCRIPT_DIR/skills/generic/$skill/SKILL.md" "$CODEX_HOME_DIR/skills/$skill/SKILL.md" "codex-skills/$skill"; then
                ((CODEX_SKILLS_UPDATED++)) || true
            else
                ((CODEX_SKILLS_SKIPPED++)) || true
            fi
        else
            if safe_copy "$SCRIPT_DIR/skills/$skill/SKILL.md" "$CODEX_HOME_DIR/skills/$skill/SKILL.md" "codex-skills/$skill"; then
                ((CODEX_SKILLS_UPDATED++)) || true
            else
                ((CODEX_SKILLS_SKIPPED++)) || true
            fi
    ...
    if [ "$CODEX_SKILLS_SKIPPED" -gt 0 ]; then
        info "Codex skills: $CODEX_SKILLS_UPDATED updated, $CODEX_SKILLS_SKIPPED skipped (locally modified)"
    else
        info "Codex skills: $CODEX_SKILLS_UPDATED installed to $CODEX_HOME_DIR/skills/"
    fi
else
    info "Codex CLI not found; skipping Codex skill install"
fi
